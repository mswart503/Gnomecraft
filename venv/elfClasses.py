...
class Board:
    def __init__(self, cardsinplay, totalcardlist, decklist):
        ...
        self.myselectedCardslot = None #Cardslot(yellow, 0, 0, 0, 0, None, True)
        self.opselectedCardslot = None #Cardslot(yellow, 0, 0, 0, 0, None, True)
    ...
    def redrawwindow(self, myselectedcard=None, opselectedcard=None):
        ...
        try:
            ...
            pass
        if self.opselectedCardslot is not None:
            self.opselectedCardslot.draw(self.window, outline=True, char=True, selected=True)
    ...
    def playopcard(self, cardtoplay):
        if cardtoplay not in self.opfield.fieldArray:
            ...
            if cardtoplay == self.opselectedCardslot:
                self.opselectedCardslot = None
            self.redrawwindow()
    ...
    def battlephase(self):
        ...
        if self.dayPowerAdvantage == "my":
            ...
            # updates card processing in case cards on field changed
            opcardstoplay = self.opfield.priorityArrays[0].copy()
            opcurlength = len(opcardstoplay)
                        ...
                        for cardpick in cardstoplay:
                            if cardpick.isOver(pos):
                                ...
                                try:
                                    ...
                                    pass
                                for removal_check in cardstoplay:
                                    if removal_check not in self.myfield.fieldArray and removal_check not in self.myfield.cardondeck:
                                        cardstoplay.remove(removal_check)
                                clicklisten = True
                                self.blankalltexts()
            ...
            if choicelength == 1:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    for removal_check in cardstoplay:
                        if removal_check not in self.opfield.fieldArray and removal_check not in self.opfield.cardondeck:
                            cardstoplay.remove(removal_check)
                except:
                    break

    def aiDecision(self, abilityid, cardslot, defendingCard=None):
        cardslot.strength = int(cardslot.strength)
        ...
        if abilityid == "gorgon":
            ...
            attackondeath = False
            cardslot.strength = int(cardslot.strength)
            attackerdmg = int(cardslot.strength)
            defenderdmg = 0
            for card in self.myfield.fieldArray:
                card.strength = int(card.strength)
    ...
    def meleeresolution(self, attacker, attackerside, defender, defenderside, attackingmodifiertype=None, attackingmodifier=0, defendingmodifiertype=None, defendingmodifier=0):
        ...
        if attackingmodifiertype == None:
            ...
            attackerdmg = math.ceil(attackerstr * attackingmodifier)
        elif attackingmodifiertype == "add":
            attackerdmg = attackerstr + attackingmodifier
        ...
        if defendingmodifiertype == None:
            ...
            defenderdmg = math.ceil(defenderstr * defendingmodifier)
        elif defendingmodifiertype == "add":
            defenderdmg = defenderstr + defendingmodifier
        elif attackingmodifiertype == "subtract":
            defenderdmg = defenderstr - defendingmodifier
        ...
        if attackerresolution == "remain" and defenderresolution == "remove":
            ...
            defenderfield = self.cardremoval(defenderfield, defender)
            self.endofRoundArray.append(["melee", attacker, attackerside])
            self.clicktocontinue()
            ...
            attackerfield = self.cardremoval(attackerfield, attacker)
            self.endofRoundArray.append(["melee", defender, defenderside])
            self.clicktocontinue()
...
"""
    ...
    def endofround(self):
        if self.endofRoundArray is not []:
            # syntax: [Name of action in STR, Card action is performed on in CARD, Which side of the field STR either "op" or "my", optional number variable]
            actName = card = whoscard = number = None
